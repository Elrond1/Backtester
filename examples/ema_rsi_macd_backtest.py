"""
Backtest: EMA 200 + RSI 14 + MACD (12, 26, 9) on BTC/USDT
- Capital : $10 000
...
# ── Custom bar-by-bar simulation with dynamic SL/TP ──────────────────────────
...
def run_dynamic_backtest(
    ...
    SL  = swing low  (long) / swing high (short) — pre-computed, no look-ahead.
    TP  = entry ± rr_ratio × |entry − SL|.
    SL check happens before TP on the same bar (conservative).
    After a forced exit the next bar is skipped (no re-entry on same candle).
    """
    ...
    for i in range(n):
        ...
        if in_trade:
            ...
            # Exit only on OPPOSITE signal or final bar (not on flat 0)
        ...
        if not in_trade and not block_entry and sig != 0:
            ...
            # Sanity: skip if SL is invalid or absurdly wide (>15%)
...
def print_trade_breakdown(result: BacktestResult) -> None:
    ...
    by_side   = trades.groupby("side")["pnl_pct"].agg(["count", "mean", lambda x: (x > 0).mean() * 100])
...
if __name__ == "__main__":
    ...
    for tf in ["1h", "15m"]:
        ...
        html   = f"ema_rsi_macd_{tf}.html"
        plot_backtest(
            result, df,
            indicators={"EMA 200": ema200},
            title=f"{SYMBOL} {tf} — EMA200 + RSI14 + MACD | 2020→ | SL swing / TP 1:2",
            save_html=html,
        )
        print(f"\n  Chart → {html}")
